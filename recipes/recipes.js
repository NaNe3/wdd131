...
let searchOptions = document.querySelector('#recipe-search-options')
...
function recipeTemplate(recipe) {
  return `<article class="recipe-card">
    ...
    <div class="recipe-content">
      <p class="category">${recipe.tags[0] ?? 'Recipe'}</p>
      <h2>${recipe.name}</h2>
      <p class="rating" role="img" aria-label="${recipe.rating} out of 5 stars">
...
function search(event) {
  ...
  let recipeQuery = searchInput.value.trim().toLowerCase()

  let filteredRecipes = recipes.filter((recipe) => {
    return recipe.name.toLowerCase().includes(recipeQuery) ||
...
function loadSearchOptions() {
  ...
  recipes.forEach((recipe) => {
    ...
    recipe.tags.forEach((tag) => options.add(tag))
  })

  searchOptions.innerHTML = ''
  options.forEach((optionValue) => {
    searchOptions.innerHTML += `<option value="${optionValue}"></option>`
  })
}
